...
Usage:
  .venv/bin/python robustness/current/calibrate_tolerances.py \
...
# Must match check_invariants.py: the current panel drops PCA_dim_90pct
# and lPCA (see metrics.py DROPPED_FROM_PANEL).
...
def main() -> None:
    ...
    # ---- inputs must exist before the expensive work --------------------
    if not (BASELINE / f"per_role_panel_L{L}.csv").exists():
        raise SystemExit(f"no baseline panel at {BASELINE}. Bootstrap the "
                         f"current tier first (see robustness/README.md).")
    if not PINS_FILE.exists():
        raise SystemExit(f"no pins at {PINS_FILE}. Run "
                         f"check_invariants.py --full --pin first.")
    ...
    tol = {k: round_up_2sig(max(2 * s, FLOORS[floor_of[k]]))
           for k, s in sd.items()}
    # Merge, never replace: keys not recomputed this run (e.g. the calib
    # tolerance when --calib-glob is omitted) keep their calibrated value.
    pins["tol"] = {**pins.get("tol", {}), **tol}
    pins["tol_provenance"] = {
