...
assistant-axis 01 script: that one takes skdim's 20-neighbour ``fit`` default,
which on a 25-point cloud uses 20 of the 24 available neighbours and makes a
"local" estimator global. ``manifold.idim`` passes the neighbourhood where skdim
reads it, ``fit(n_neighbors=min(10, n-2))``, for exactly this small-n case.
